...
all.scars.g1.1 <- read.table("./Data/2017_10X_7/H5_scar_reads_over1.txt",
                             sep = "\t", stringsAsFactors = F)
colnames(all.scars.g1.1)[-1] <- c("Barcode", "UMI", "Location", "Sequence")
all.scars.g1.1$Library <- "H5"
...
# Undersequencing filter
ggplot(scars.output[scars.output$Library == "H5", ]) +
  geom_histogram(aes(x = Reads), binwidth = 10) +
  scale_x_continuous(limits = c(-10, 100))
min.scar.reads.H5 <- 50
scars.output.2.H5 <- scars.output[scars.output$Reads >= min.scar.reads.H5 & 
                                    scars.output$Library == "H5", ]
ggplot(scars.output[scars.output$Library == "B5", ]) +
  geom_histogram(aes(x = Reads), binwidth = 50) +
  scale_x_continuous(limits = c(-50, 2000))
min.scar.reads.B5 <- 250
scars.output.2.B5 <- scars.output[scars.output$Reads >= min.scar.reads.B5 & 
                                    scars.output$Library == "B5", ]
ggplot(scars.output[scars.output$Library == "P5", ]) +
  geom_histogram(aes(x = Reads), binwidth = 10) +
  scale_x_continuous(limits = c(-10, 1000))
min.scar.reads.P5 <- 200
scars.output.2.P5 <- scars.output[scars.output$Reads >= min.scar.reads.P5 & 
                                    scars.output$Library == "P5", ]
scars.output.2 <- rbind(scars.output.2.B5, scars.output.2.H5, scars.output.2.P5)
